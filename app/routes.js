...
module.exports = function(app) {
	...
	app.post('/searchbysubject', function(req, res) {
		...
		User.find({'isTutor': true, 'teach_subjects.subject': subject, 'teach_subjects.level': level}
			...
			teachers.forEach(function(teacher) {
				var check_subject_and_level = false;
				teacher.teach_subjects = teacher.teach_subjects.filter(function(teach_subject) {
					return subject.test(teach_subject.subject) && level.test(teach_subject.level);
				});
				res_teachers.push(teacher);
			})
			res.json(res_teachers);
		});	
	});
}
